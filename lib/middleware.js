...
exports.checkCache = function(req, rsp, next){
  ...
  var cached = cache.getFile(normalizedPath)
  console.log(cached)
  if(normalizedPath && cached){
    return _sentData(rsp, cached)
  } else {
    return next()
  }
}
  ...
  __render(sourceFile, function(error, result){
    ...
    if(error){
      ...
      if(_.isString(result)){
        ...
        var sourceMap = result['v3SourceMap'] || result['sourceMap']
        var content = result['js'] || result['css']
        if(sourceMap){
          var sourcemap_path = normalizedPath + '.map'
          outputs.push({
              'path': sourcemap_path,
              'body': sourceMap
          })
          var sourcemapTail = "\n/*\n//@ sourceMappingURL="
              + path.basename(normalizedPath) + ".map\n*/\n"
          content = content + sourcemapTail
        }
        outputs.push({
            'path': normalizedPath,
            'body': content
      ...
      var _results = _.object(_.map(outputs, function(o, k, list){
        var outputType = polymer.helpers.outputType(o.path)
        ...
        var data = {
          ...
          'headers': {
            ...
            'Content-Length': Buffer.byteLength(o.body, charset)
          },
          'statusCode': 200
        }
        return [o.path, data]
      ...
      return _sentData(rsp, _results[normalizedPath])
    }
  })
